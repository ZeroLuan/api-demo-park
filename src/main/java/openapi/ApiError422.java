...
@ApiResponse(
        responseCode = "422",
        description = "Recurso não processável por dados de entrada inválidos - @Valid",
        content = @Content(
                mediaType = MediaType.APPLICATION_JSON_VALUE,
                schema = @Schema(implementation = ErrorMessageResponse.class)
        )
)
public @interface ApiError422 { // Quando você vê @interface, você não está criando uma "Interface comum" (como UsuarioDoc), você está criando uma Anotação Personalizada.
}
